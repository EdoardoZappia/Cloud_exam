...
# Apply the latency-one-node.yaml file to the cluster
echo "------------------- Running OSU Benchmark (latency-one-node) -------------------" >> Results/Results.txt
kubectl apply -f latency-one-node.yaml -n osu-benchmark
...
kubectl delete -f latency-one-node.yaml -n osu-benchmark
