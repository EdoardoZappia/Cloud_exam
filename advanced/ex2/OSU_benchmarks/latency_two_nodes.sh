...
# Apply the latency-two-nodes.yaml file to the cluster
echo "------------------- Running OSU Benchmark (latency-two-nodes) -------------------" >> Results/Results.txt
kubectl apply -f latency-two-nodes.yaml -n osu-benchmark
...
kubectl delete -f latency-two-nodes.yaml -n osu-benchmark
